...
import {
  Package, Users, ClipboardList, Clock, Plus, Trash2, Edit2, PackagePlus, CalendarPlus, Scan, CheckCircle2, XCircle
} from "lucide-react"
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <Tabs defaultValue="stock" className="w-full">
        <TabsList className="mb-6 grid w-full grid-cols-2 gap-1 bg-muted p-1 md:grid-cols-6">
          ...
          <TabsTrigger value="queue" className="flex items-center gap-1.5 text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
            <Scan className="h-3.5 w-3.5" /> Queue
          </TabsTrigger>
        </TabsList>
        ...
        {/* Queue / QR Verification Tab */}
        <TabsContent value="queue">
          <QueueVerification shop={shop} />
        </TabsContent>
      </Tabs>
    </div>
  )
...
function QueueVerification({ shop }: { shop: Rec }) {
  ...
  const [verificationResult, setVerificationResult] = useState<{
    ...
    message: string;
    details?: {
      consumer: { name: string; card_no: string; type: string };
  ...
  const handleVerify = async (e?: React.FormEvent, code?: string) => {
    ...
    try {
      const res = await fetch('/api/queue/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ qr_code: codeToVerify, shop_id: shop.id }),
      })
  ...
  return (
    <Card className="border-primary/20 max-w-xl mx-auto shadow-md">
      <CardHeader className="bg-primary/5 pb-3">
        <CardTitle className="flex items-center gap-2 text-base text-primary">
          <Scan className="h-5 w-5" /> QR Code Booking Verification
        </CardTitle>
      </CardHeader>
      <CardContent className="pt-6">
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="text-sm font-semibold">Enter or Scan Consumer QR Code</Label>
            <div className="flex gap-2">
              <Input
                value={qrCode}
                onChange={(e) => setQrCode(e.target.value)}
                placeholder="e.g. Q-20240101-123-9999"
                className="font-mono text-sm border-primary/30 focus-visible:ring-primary shadow-inner"
              />
              <Button
                type="button"
                variant="outline"
              ...
              <Button onClick={() => handleVerify()} disabled={verifying || !qrCode} className="bg-primary text-primary-foreground shadow-md hover:translate-y-[-1px] transition-transform">
                {verifying ? "..." : "Verify"}
              </Button>
            </div>
            <p className="text-[10px] text-muted-foreground italic">
              Use the camera to scan the QR shown on the consumer's dashboard.
            </p>
          </div>
          ...
          <QrScannerModal
            ...
            onScan={(code) => handleVerify(undefined, code)}
          />

          {verificationResult && (
            <div className={`mt-6 overflow-hidden rounded-lg border-l-4 shadow-sm transition-all duration-300 animate-in fade-in slide-in-from-top-2 ${verificationResult.success ? 'bg-india-green/10 border-india-green' : 'bg-destructive/10 border-destructive'
              }`}>
              <div className="flex items-start gap-3 p-4">
                <div className={`mt-1 rounded-full p-1.5 ${verificationResult.success ? 'bg-india-green text-white' : 'bg-destructive text-white'}`}>
                  {verificationResult.success ? <CheckCircle2 className="h-5 w-5" /> : <XCircle className="h-5 w-5" />}
                </div>
                <div className="flex-1">
                  <h3 className={`text-lg font-bold tracking-tight ${verificationResult.success ? 'text-india-green' : 'text-destructive'}`}>
                    {verificationResult.success ? 'Verification Successful' : 'Verification Failed'}
                  </h3>
                  <p className="text-sm font-medium text-foreground/80 mt-0.5">{verificationResult.message}</p>

                  {verificationResult.success && verificationResult.details && (
                    <div className="mt-4 space-y-4 rounded-md bg-white/60 dark:bg-black/20 p-4 border border-primary/10 shadow-inner">
                      <div className="grid grid-cols-2 gap-y-4 gap-x-2">
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Consumer</span>
                          <span className="block font-semibold text-primary">{verificationResult.details.consumer.name}</span>
                        </div>
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Card No</span>
                          <span className="block font-mono text-sm">{verificationResult.details.consumer.card_no}</span>
                        </div>
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Category</span>
                          <Badge variant="outline" className="border-saffron text-saffron bg-saffron/5 font-semibold text-[10px]">{verificationResult.details.consumer.type}</Badge>
                        </div>
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Allocation Period</span>
                          <div className="flex items-center gap-1 text-primary">
                            <CalendarPlus className="h-3 w-3" />
                            <span className="font-medium text-xs font-mono">{verificationResult.details.timing}</span>
                          </div>
                        </div>
                      </div>

                      <div className="mt-2 border-t border-dashed border-primary/20 pt-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Allocated Ration Items</span>
                          <Badge variant="secondary" className="text-[9px] h-4">Verified Allocation</Badge>
                        </div>
                        <ul className="space-y-2">
                          {verificationResult.details.items.map((item, i) => (
                            <li key={i} className="flex items-center justify-between rounded bg-primary/5 px-2 py-1.5 text-xs transition-colors hover:bg-primary/10">
                              <div className="flex items-center gap-2">
                                <Package className="h-3.5 w-3.5 text-muted-foreground" />
                                <span className="font-medium text-foreground">{item.name}</span>
                              </div>
                              <span className="font-bold text-india-green">{item.quantity}</span>
                            </li>
                          ))}
                        </ul>
                      </div>

                      <Button className="w-full mt-2 bg-india-green text-white hover:bg-india-green/90 shadow-md h-9 text-xs">
                        <ClipboardList className="mr-2 h-4 w-4" /> Issue Items & Close Booking
                      </Button>
                    </div>
                  )}
                </div>
